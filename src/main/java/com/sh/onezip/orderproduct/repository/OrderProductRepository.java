...
public interface OrderProductRepository extends JpaRepository<OrderProduct, Long> {
    ...
    @Query("delete from OrderProduct op where op.productLog = :merchantUid")
    ...
    @Query("from OrderProduct op where op.product.member.memberId = :bizMemberId order by op.id desc")
    Page<OrderProduct> productOrderFindAllByBizMemberId(Pageable pageable, String bizMemberId);

    @Query("from OrderProduct op where op.product.member.memberId = :bizMemberId")
